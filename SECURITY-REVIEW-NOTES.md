...
- **Flagged in:** Phase 6
- **Risk:** All tables (`publishers`, `sites`, `pricing_rules`, `policy_rules`, `transactions`) are plain Postgres tables with no row-level security, accessed only through Prisma from server-side Next.js code (no PostgREST or other direct-client-to-Postgres path in this architecture, so there's no equivalent `anon`/`authenticated` role to backstop with RLS in the first place). That means the database itself enforces no per-publisher boundary at all — every `requirePublisher()`/`requireOwnedSite()` call in application code is the *only* thing stopping one publisher's session from reading or modifying another's sites, pricing, or transaction history. A bug in any of those checks, or a leak of `DATABASE_URL`, is a full cross-tenant breach, not a scoped one.
- **Suggested fix:** At minimum, add integration tests that assert cross-tenant access is rejected for every mutating action, so a regression here fails CI instead of shipping silently. Postgres RLS is still an option if a direct-DB access path is ever added (e.g. a future client-side query layer), but as long as every access goes through Prisma from trusted server code, it wouldn't add a real second boundary today.
